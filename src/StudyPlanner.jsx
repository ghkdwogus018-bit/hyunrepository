...
const COURSES = [
  ["영어", "심슨 영기싹(노베이스)", 32, 1, 4], ["영어", "심슨 구문", 20, 4, 7], ["영어", "심슨 문법", 28, 7, 12],
  ["영어", "심슨 독해", 28, 11, 16], ["영어", "기출의 품격·유형별", 36, 17, 26], ["영어", "심슨 동형·파이널", 24, 37, 46],
  ["국어", "시간단축 공문서·문법독해", 18, 2, 5], ["국어", "이선재 올인원", 36, 5, 11], ["국어", "논리·독해 훈련(1일3독)", 24, 9, 13],
  ["국어", "선재 예상 기출서", 35, 17, 22], ["국어", "신유형 고난도 추론", 14, 21, 26], ["국어", "실전형 훈련 모의고사", 20, 29, 36], ["국어", "합격선 파이널·봉투", 10, 41, 47],
  ["행정법", "행정법으로의 초대(입문)", 18, 1, 3], ["행정법", "써니 행정법총론 올인원", 76, 3, 16], ["행정법", "써니 기출문제 풀이", 40, 17, 26],
  ["행정법", "핵심집약", 30, 21, 30], ["행정법", "단원별·실전 동형", 24, 37, 46],
  ["행정학", "행정학 입문 특강", 14, 2, 3], ["행정학", "황철곤 기본서 올인원", 56, 6, 16], ["행정학", "황철곤 핵심 기출", 36, 17, 26],
  ["행정학", "출제범위 확장(고난도)", 14, 23, 28], ["행정학", "황철곤 동형·최종병기", 24, 37, 46],
...
function generateWeek(W) {
  const tasks = []; let rr = 0;
  COURSES.forEach((c) => { const r = c.alloc[W]; if (r) for (let l = r[0]; l <= r[1]; l++) { tasks.push({ id: `w${W}c${c.ci}l${l}`, sub: c.sub, ci: c.ci, label: `${c.name} 제${l}강`, dDay: rr % 6 }); rr++; } });
  for (let d = 0; d < 6; d++) {
...
const MIN_DAILY = "영어 단어 + 국어 1일 1독 + 인강 1강 복습";
